...
stock_table = soup.find(class_='table table-striped') 
urls = []

#find all 'a' from the specified location
#extract attribute inside 'a'
for a in stock_table.find_all('a'):   
    urls.append(a.attrs['href'])      

#only links at even number index is required
even_list = [] 
...
even_link = [link_front + s for s in even_list]       

#create lists to store crawled data
fullname_list = []
name_list = []
category_list = []
w52_list = []
...
#pass crawled data into a dataframe
import pandas as pd
...
dataframe.to_csv('klse_FEB.csv', index = False, sep = ',')        